...
class Goods extends Base{
	...
	public function saleByPage(){
		...
		$rs = $this->alias('g')->join('__SHOPS__ s','g.shopId=s.shopId','left')
		    ->where($where)
			->field('goodsId,goodsName,goodsSn,saleNum,shopPrice,supplyPrice,marketPrice,isDouble,goodsReward,goodsImg,goodsPoint,goodsRate,goodsDiscount,g.shopId,goodsImg,s.shopName,goodsCatIdPath')
			->order($order)
    ...
    public function auditByPage(){
		...
		$rs = $this->alias('g')->join('__SHOPS__ s','g.shopId=s.shopId','left')
		    ->where($where)
			->field('goodsId,goodsName,goodsSn,saleNum,shopPrice,supplyPrice,marketPrice,isDouble,goodsReward,goodsImg,goodsPoint,goodsRate,goodsDiscount,s.shopName,s.shopId,goodsCatIdPath')
			->order($order)
			->paginate(input('limit/d'))->toArray();
        foreach ($rs['data'] as $key => $v){
			$rs['data'][$key]['verfiycode'] =  WSTShopEncrypt($v['shopId']);
