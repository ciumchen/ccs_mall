...
class Users extends Base{
    ...
    protected $beforeActionList = [
          'checkAuth' =>  ['except'=>'api_login,checklogin,login,register,getverify,toregister,forgetpass,forgetpasst,forgetpasss,findpass,getfindphone,resetpass,getphoneverifycode,checkuserphone']// 访问这些except下的方法不需要执行前置操作
    ];
    /**
     * 会员登录页
     */
    public function login(){
        header("Location:".config('app.member_sys_domain').'/login/login_page');
        exit;
        //如果已经登录了则直接跳去用户中心
    ...
    function api_login()
    {
        ...
        $string = base64_decode($t);
        $encryptKey = '5c1f5927bfc2992f831ce9ed31d4f12d';

        //加密
        require Env::get('root_path') . 'extend/org/Base64.php';
        $base64 = new \org\Base64();
        $string_info = $base64->unlock_url($string, $encryptKey);

        if (!$string_info) {
            return WSTReturn("参数不合法", -1);
        }

        //step1 获取cookie信息
        $info = json_decode($string_info, true);
        if (!$info) {
            return WSTReturn("获取信息失败", -1);
        }
        ...
        $USER = session('WST_USER');
        if(empty($USER) || (isset($USER['loginName']) && $USER['loginName']!= $mobile)) {
            //step3 查找用户表是否存在该用户
            $m = new MUsers();
            $userRow = $m->get(['userPhone' => $mobile]);
            ...
            if (!$userRow) {
                ...
                $user_id = $m->insertGetId($data);

            } else {
                $user_id = $userRow['userId'];
                if($userRow['wxOpenId'] == '' && $openid!=''){
                    $m->save(['wxOpenId'=>$openid],['userId'=>(int)$user_id]);
                }
            }
            $userRow = $m->get(['userId'=>(int)$user_id]);

            //step5 同步登陆该用户
            $m->afterLogin($userRow);
        }

        //step6 如果没有重定向，则跳转到首页
    ...
    public function toRegister(){
        header("Location:".config('app.member_sys_domain').'/Register/register_page');
        exit;
	...
	public function index(){
        header("Location:".config('app.member_sys_domain'));
        exit;
