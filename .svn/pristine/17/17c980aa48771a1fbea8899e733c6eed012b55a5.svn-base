...
class Integrals extends Base{
    ...
    public function installMenu(){
		...
		try{
			...
			//管理员后台
			$rs = Db::name('menus')->insert(["parentId"=>93,"menuName"=>"金币专区","menuSort"=>1,"dataFlag"=>1,"isShow"=>1,"menuMark"=>"integral"]);
			if($rs!==false){
				$datas = [];
				$parentId = Db::name('menus')->getLastInsID();
				$datas[] = ["menuId"=>$parentId,"privilegeCode"=>"INTEGRAL_TGHD_00","privilegeName"=>"查看金币专区活动","isMenuPrivilege"=>1,"privilegeUrl"=>"/addon/integral-goods-pageByAdmin","otherPrivilegeUrl"=>"/addon/integral-goods-pageQueryByAdmin,/addon/integral-goods-pageAuditQueryByAdmin","dataFlag"=>1,"isEnable"=>1];
				$datas[] = ["menuId"=>$parentId,"privilegeCode"=>"INTEGRAL_TGHD_04","privilegeName"=>"金币专区商品操作","isMenuPrivilege"=>0,"privilegeUrl"=>"","otherPrivilegeUrl"=>"/addon/integral-goods-allow,/addon/integral-goods-illegal","dataFlag"=>1,"isEnable"=>1];
				$datas[] = ["menuId"=>$parentId,"privilegeCode"=>"INTEGRAL_TGHD_03","privilegeName"=>"删除金币专区商品","isMenuPrivilege"=>0,"privilegeUrl"=>"/addon/integral-goods-delByAdmin","otherPrivilegeUrl"=>"","dataFlag"=>1,"isEnable"=>1];
	...
	public function add(){
	    # 连接会员中心数据库
        $date = date('Ymd');
        $goldPrice = Db::connect('member')->table('m_gold_price_day')->where('day','=',$date)->find();
        $gold = $goldPrice['gold_price'];
		...
		if($data['startTime']=='' || $data['endTime']=='')return WSTReturn('请选择有效金币专区商品时间');
		if(WSTStrToTime($data['startTime']) > WSTStrToTime($data['endTime']))return WSTReturn('金币专区商品开始时间不能大于结束时间');
		//判断是否已经存在同时间的金币专区商品
		$where = [];
		...
		if($rn>0)return WSTReturn('该商品已存在另外一个相同时段的金币专区活动中');
		WSTUnset($data,'id,cat_0,illegalRemarks');
		$shopId = (int)$goods['shopId'];
        $shopPrice = $goods['shopPrice'];
        $integralNum = round($goods['shopPrice'] / $gold, 2);
		$data['shopId'] = $shopId;
		$data['goodsPrice'] = $shopPrice;
        $data['integralNum'] = $integralNum;
		$data['dataFlag'] = 1;
		$data['orderNum'] = 0;
	...
	public function edit(){
		...
		# if((float)$data['goodsPrice']<0)return WSTReturn('商品价格不能小于0');
		# if((int)$data['integralNum']<=0)return WSTReturn('积分数必须大于0');
		if((int)$data['totalNum']<=0)return WSTReturn('金币专区数量必须大于0');
		if($data['startTime']=='' || $data['endTime']=='')return WSTReturn('请选择有效金币专区商品时间');
		if(WSTStrToTime($data['startTime']) > WSTStrToTime($data['endTime']))return WSTReturn('金币专区商品开始时间不能大于结束时间');
		//判断是否已经存在同时间的金币专区
		$id = $data['id'];
	...
	public function addCart($uId=0){
		...
		if($userData['gold'] < ($goods["integralNum"]*$cartNum)){
			return WSTReturn("金币不足", -1);
		}
        $carts = [];
        $carts['id'] = $id;
	...
	public function checkGoodsSaleSpec($id){
		...
		//判断金币专区是否过期
		$time = time();
		if(strtotime($goods['startTime']) > $time)return WSTReturn('对不起，金币专区活动尚未开始');
		if(strtotime($goods['endTime']) < $time)return WSTReturn('对不起，您来晚了，金币专区活动已结束');
	...
	public function getCarts(){
		...
		$carts['goodsMoney'] = round($carts['goodsMoney'] + $rs['shopPrice'] * $rs['cartNum'], 2);
		$goodsTotalMoney = round($goodsTotalMoney + $rs['shopPrice'] * $rs['cartNum'], 2);
		$totalIntegralNum = round($totalIntegralNum + $rs['integralNum'] * $rs['cartNum'], 2);
	...
	public function submitByVirtual($carts,$orderSrc = 0,$uId){
		...
		if($userData["gold"]<$totalIntegralNum){
			return WSTReturn('您的金币不足，不能购买');
		}
		//生成订单
		Db::startTrans();
		try{
			...
			if(count($cards)<$goods['cartNum'])return WSTReturn("下单失败，金币专区商品库存不足");
			//修改库存
			...
			if(false !== $result){
				...
				if($order['useScore']>0){
					...
					$score['dataRemarks'] = "交易订单【".$orderNo."】使用金币".$order['useScore']."个";
					$score['scoreType'] = 0;
	...
	public function submitByEntity($carts,$orderSrc = 0,$uId=0){
        ...
        # halt($userName);
        # 连接会员中心数据库
        $userData = Db::connect('member')->table('m_users')->where('mobile', $userName)->field('uid,gold,image_url,address')->find();
        # halt($userData['address']);
        ...
        $isUseScore = round(input('isUseScore'), 2);
        $useScore = round(input('useScore'), 2);
		...
		try{
			...
			$order['shopId'] = $carts['shopId'];
			$order['payType'] = $payType;
			$order['goodsMoney'] = isset($carts['goodsMoney']) ? 0 : 0;
			//计算运费和总金额
			...
			//实付金额要减去积分兑换的金额
			$order['realTotalMoney'] = round($order['totalMoney'] - $order['scoreMoney'], 2);
			$order['needPay'] = $order['realTotalMoney'];
			$order['orderCode'] = 'integral';
			$order['orderCodeTargetId'] = $carts['id'];
			$order['extraJson'] = json_encode(['id'=>$carts['id']]);
			...
			if(false !== $result){
				...
				if($order['useScore']>0){
					$score = [];
                    $score['uid'] = $userData['uid'];
                    $score['gold'] = $order['useScore'];
                    $score['type'] = 13;
				    /*$score['userId'] = $userId;
					$score['score'] = $order['useScore'];
					$score['dataSrc'] = 1;
	...
	public function submit($orderSrc = 0,$uId=0){
		...
		//检测时间过了没有
		$time = time();
		if(strtotime($carts['carts']['goods']['startTime']) > $time)return WSTReturn('对不起，金币专区活动尚未开始');
		if(strtotime($carts['carts']['goods']['endTime']) < $time)return WSTReturn('很抱歉，您来晚了，金币专区活动已结束');
