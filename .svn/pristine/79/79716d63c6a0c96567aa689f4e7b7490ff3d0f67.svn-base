...
class Goods extends CGoods{
	...
	public function saleByPage(){
		...
		$rs = $this->alias('m')
		    ->where($where)
			->field('goodsId,goodsName,goodsImg,goodsType,goodsSn,isTrain,isSale,isBest,isHot,isNew,isRecom,isDouble,goodsStock,saleNum,shopPrice,goodsDiscount,isSpec')
			->order('saleTime', 'desc')
    ...
    public function storeByPage(){
		...
		$rs = $this->alias('m')
		    ->where($where)
		    ->where('goodsStatus','<>',-1)
			->field('goodsId,goodsName,goodsImg,goodsType,goodsSn,isTrain,isSale,isBest,isHot,isNew,isRecom,goodsStock,saleNum,shopPrice,supplyPrice,goodsDiscount,isSpec')
			->order('saleTime', 'desc')
			->paginate(input('pagesize/d'))->toArray();
        foreach ($rs['data'] as $key => $v){
			$rs['data'][$key]['verfiycode'] =  WSTShopEncrypt($shopId);
	...
	public function illegalByPage(){
		...
		$rs = $this->alias('m')
		    ->where($where)
			->field('goodsId,goodsName,goodsImg,goodsType,goodsSn,isTrain,isSale,isBest,isHot,isNew,isRecom,illegalRemarks,goodsStock,saleNum,shopPrice,supplyPrice,goodsDiscount,isSpec')
			->order('saleTime', 'desc')
			->paginate(input('pagesize/d'))->toArray();
		foreach ($rs['data'] as $key => $v){
			$rs['data'][$key]['verfiycode'] = WSTShopEncrypt($shopId);
	...
	public function add(){
         ...
         */
        if($data['goodsRate'] == 0){
            $point = ($data['shopPrice']-$data['supplyPrice']*1.16)*0.9;
        }elseif($data['goodsRate'] == 1){
            $point = ($data['shopPrice']-$data['supplyPrice']/1.03*1.16)*0.9;
        }elseif($data['goodsRate'] == 2){
            $point = ($data['shopPrice']-$data['supplyPrice'])*0.9;
        }elseif($data['goodsRate'] == 3){
            $point = ($data['shopPrice']-$data['supplyPrice'])*0.9;
        }else{
            $point = 0;
        }
        ...
        $data['goodsPoint'] = number_format($point,'2','.','');
        $data['goodsReward'] = $data['goodsPoint'];

        # 商品折扣
        $data['goodsDiscount'] = $data['supplyPrice'] / $data['shopPrice'];
			...
			if(false !== $result){
    	        ...
    	        if($data['goodsType']==0 && $specsIds!=''){
		    		...
		    		foreach ($specsArray as $v){
                        ...
                        # $sitem['supplyPrice'] = (float)Input('supplyPrice_'.$v);
                        # $sitem['specPrice'] = (float)Input('specPrice_'.$v);
		    			$sitem['dataFlag'] = 1;
		    		...
		    		foreach ($specsIds as $v){
                        ...
                        $gspec['supplyPrice'] = (float)Input('supplyPrice_'.$v);
	...
	public function getById($goodsId){
		...
		if(!empty($rs)){
			...
			$rs['spec1'] = $spec1;
			//获取销售规格
			$rs['saleSpec'] = Db::name('goods_specs')->where('goodsId',$goodsId)->field('id,isDefault,productNo,specIds,marketPrice,supplyPrice,specPrice,goodsDiscount,specStock,warnStock,saleNum')->select();
			//获取属性值
    ...
    public function getBySale($goodsId){
        # 连接会员中心数据库
        /*$date = date('Ymd');
        $goldPrice = Db::connect('db_config_member')->table('m_gold_price_day')->where('day','=',$date)->find();
        ...
        if(!empty($rs)){
            ...
            /*if($rs['isGold'] == 1)
            {
                # $rs['marketPrice'] = round($rs['marketPrice'] / $gold, 2);
                $rs['shopPrice'] = round($rs['shopPrice'] / $gold, 2);
            }*/
			...
			if($rs['isSpec']==1){
				...
				//获取销售规格
				$sales = Db::name('goods_specs')->where('goodsId',$goodsId)->field('id,isDefault,productNo,specIds,marketPrice,supplyPrice,specPrice,goodsDiscount,specStock')->select();
				if(!empty($sales)){
	...
	*/
	public function changSaleStatus(){
		$shopId = (int)session('WST_USER.shopId');
		$allowArr = ['isHot','isNew','isBest','isRecom','isDouble'];
		$is = input('post.is');
	...
	public function pageQuery($goodsCatIds = []){
        ...
        $isDouble = input('isDouble/d');
        $isGlobal = input('isGlobal/d');
        $isTrain = input('isTrain/d');
		$where = $where2 = [];
		$where['goodsStatus'] = 1;
		$where['g.dataFlag'] = 1;
		$where['isSale'] = 1;
        ...
        if($isDouble==1)$where['isDouble'] = 1;
        if($isGlobal==1)$where['isGlobal'] = 1;
        if($isTrain==1)$where['isTrain'] = 1;
		...
		$list = Db::name("goods")->alias('g')->join("__SHOPS__ s","g.shopId = s.shopId")
			...
			->field('goodsId,goodsName,goodsSn,goodsStock,isNew,saleNum,shopPrice,marketPrice,supplyPrice,goodsDiscount,isSpec,goodsImg,appraiseNum,visitNum,s.shopId,shopName,isSelf,isFreeShipping,gallery')
		...
		/*if($isGold == 1)
		{
		    ...
		    {
                # $list['data'][$key]['marketPrice'] = round($list['data'][$key]['marketPrice'] / $gold, 2);
                $list['data'][$key]['shopPrice'] = round($list['data'][$key]['shopPrice'] / $gold, 2);
            }
