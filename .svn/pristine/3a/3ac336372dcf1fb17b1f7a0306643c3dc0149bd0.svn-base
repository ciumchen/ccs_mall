...
class OrderRefunds extends Base{
	...
	public function complateOrderRefund($obj){
        ...
        try{
			...
			if($order->useScore>0){
				$score = [];
                # $score['uid'] = $order->userId;
                $score['gold'] = $order->useScore;
                $score['type'] = 14;
                $score['mobile'] = $order->user_mobile;
				# $score['uid'] = $order->userId;
				# $score['score'] = $order->useScore;
				# $score['dataSrc'] = 4;
				# $score['dataId'] = $refund['id'];
				$score['dataRemarks'] = "返还订单【".$order->orderNo."】金币".$order->useScore."个";
				# $score['scoreType'] = 1;
			...
			if((int)WSTConf('CONF.wxenabled')==1){
				...
				$params['MONEY'] = $refund->backMoney.(($order['useScore']>0)?("【退回金币：".$order['useScore']."】"):"");
	...
	public function toExport(){
		...
		->setCellValue('K1', '申请退款金额')->setCellValue('L1', '退还金币')->setCellValue('M1', '申请时间')->setCellValue('N1', '退款状态')->setCellValue('O1', '退款备注');
