...
class Orders extends Base{
	...
	public function pageQuery($orderStatus = 10000,$isAppraise = -1){
	    ...
	    if(count($page['data'])>0){
	    	 foreach ($page['data'] as $key => $v){
	    	 	 ...
	    	 	 #TODO 通过订单ID，获取订单商品表中的goods_id 和 商品表 goods_id 关联 得到供货价
                 $supplyPrice = Db::name('order_goods')->alias('g')
                     ->join('__GOODS__ gs', 'gs.goodsId=g.goodsId', 'left')
                     ->where('orderId',$v['orderId'])
                     ->sum('supplyPrice');
                 $page['data'][$key]['supplyPrice'] = $supplyPrice;
	    	 }
	    }
	    return $page;
	}
	...
	public function toExport(){
		...
		if(count($page)>0){
			...
			foreach ($page as $key => $v){
                ...
                $supplyPrice = Db::name('order_goods')->alias('g')
                    ...
                    ->where('orderId',$v['orderId'])
                    ->sum('supplyPrice');
                $page[$key]['supplyPrice'] = $supplyPrice;
			}
		}
		...
		->setCellValue('Q1', '订单总金额')->setCellValue('R1', '运费')->setCellValue('S1', '实付金额')->setCellValue('T1', '供货价格')->setCellValue('U1', '下单时间')->setCellValue('V1', '付款时间')
        ->setCellValue('W1', '发货时间')->setCellValue('X1', '收货时间')->setCellValue('Y1', '取消/拒收原因')->setCellValue('Z1', '是否退款');
		...
		for ($row = 0; $row < count($page); $row++){
			...
			->mergeCells('X'.$i2.':X'.$i)->mergeCells('Y'.$i2.':Y'.$i)->mergeCells('Z'.$i2.':Z'.$i);
            ...
            ->setCellValue('T'.$i2, $page[$row]['supplyPrice'])->setCellValue('U'.$i2, $page[$row]['createTime'])->setCellValue('V'.$i2, $page[$row]['payTime'])->setCellValue('W'.$i2, $page[$row]['deliveryTime'])
			->setCellValue('X'.$i2, $page[$row]['receiveTime'])->setCellValue('Y'.$i2, $page[$row]['logContent'])->setCellValue('Z'.$i2, ($page[$row]['isRefund']==1)?'是':'');
			$objPHPExcel->getActiveSheet()->getStyle('F'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			$objPHPExcel->getActiveSheet()->getStyle('Y'.$i2)->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
