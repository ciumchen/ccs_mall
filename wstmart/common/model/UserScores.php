...
class UserScores extends Base{
    ...
    public function add($score,$isAddTotal = false){
        ...
        if($score['type'] == 14)
        {
            ...
            # $uid = Db::connect('member')->table('m_users')->where('mobile', $score['mobile'])->field('uid')->find();
            Db::connect('member')->table('m_user_change_gold')->insert(['uid' => $user['uid'],'status' => 1,'gold' => $score['gold'],'type' => 14]);
            # if($isAddTotal)$user->userTotalScore = $user->userTotalScore+$score['score'];
        } else
        {
            $user['gold'] = $user['gold'] - $score['gold'];
            Db::connect('member')->table('m_user_change_gold')->insert(['uid' => $score['uid'],'status' => 1,'gold' => -$score['gold'],'type' => 13,'order_id'=>$score['orderNo']]);
        }
