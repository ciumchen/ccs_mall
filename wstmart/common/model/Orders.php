...
class Orders extends Base{
	...
	public function quickSubmit($orderSrc = 0, $uId=0){
		...
		try{
             ...
             * 会员的手机号，关联奖金系统
             */
            $order['user_mobile'] = session('WST_USER.userPhone');
			...
			foreach ($carts['carts'] as $ckey =>$shopOrder){
				...
				if(false !== $result){
					...
					foreach ($shopOrder['list'] as $gkey =>$goods){
                         ...
                         * 得到商品的积分
                         */
                        $orderGoods['goodsPoint'] = number_format(getGoodsPoint($goods['goodsId']) * $goods['cartNum'],'2','.','');
                        $orderGoods['goodsReward'] = number_format(getGoodsPoint($goods['goodsId'],'reward') * $goods['cartNum'],'2','.','');;
	...
	public function toExport(){
		...
		for ($row = 0; $row < count($page); $row++){
			...
			for ($row2 = 0; $row2 < $goodsn; $row2++){
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('A'.$i3, $page[$row]['orderNo'])->setCellValue('B'.$i3, $page[$row]['status'])->setCellValue('C'.$i3, $page[$row]['userName'])->setCellValue('D'.$i3, $page[$row]['userAddress'])
                    ->setCellValue('E'.$i3, $page[$row]['userPhone'])->setCellValue('F'.$i3, $page[$row]['payTypeName'])->setCellValue('G'.$i3, ($page[$row]['payFrom'])?WSTLangPayFrom($page[$row]['payFrom']):'')->setCellValue('H'.$i3, $page[$row]['deliverType'])
                    ->setCellValue('I'.$i3, $page[$row]['orderRemarks'])->setCellValue('J'.$i3, $page[$row]['invoiceArr'])->setCellValue('N'.$i3, $page[$row]['totalMoney'])->setCellValue('O'.$i3, $page[$row]['deliverMoney'])->setCellValue('P'.$i3, $page[$row]['realTotalMoney'])
                    ->setCellValue('Q'.$i3, $page[$row]['createTime'])->setCellValue('R'.$i3, $page[$row]['payTime'])->setCellValue('S'.$i3, $page[$row]['deliveryTime'])->setCellValue('T'.$i3, $page[$row]['receiveTime'])
                    ->setCellValue('U'.$i3, $page[$row]['logContent'])->setCellValue('V'.$i3, ($page[$row]['isRefund']==1)?'是':'');
