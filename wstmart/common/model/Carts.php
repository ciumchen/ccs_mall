	...
	public function addCart(){
		...
		if($chk['data']['goodsType']==0){
		    ...
		    if($goodsId == 2746){
                ...
                $user = Db::connect('member')->table('m_users')->field('uid,level')->where(['mobile' => $userName, 'status' => 1])->find();
                if(!$user)return WSTReturn("无效的会员", -1);
                if($user && $user['level'] >= 3){
                    $count = Db::connect('member')->table('m_user_help_plan')->where(['uid' => $user['uid']])->count();
                    if($count == 0 ){ //V3等级的必须是参加了帮扶计划
                        return WSTReturn("您没有参加帮扶计划无法购买", -1);
                    }
                }
            }
