 ...
 * 得到商品的积分
 */
function getGoodsPoint($goodsId,$type='point'){
    $goods = Db::name('goods')->where('goodsId',$goodsId)->field('goodsPoint,isDouble,goodsReward')->find();
    if(empty($goods)){
        return 0;
    }else{

        if($type == 'point'){
            //todo 积分翻倍在会员系统处理，不然积分就是利润，利润翻倍了bug。
            if($goods['isDouble'] == 1){
                //return $goods['goodsPoint'] >=0 ? $goods['goodsPoint'] * 2 : 0;
                return $goods['goodsPoint'] >=0 ? $goods['goodsPoint'] : 0;
            }else{
                return $goods['goodsPoint'] >=0 ? $goods['goodsPoint'] : 0;
            }
        }else{
            return $goods['goodsReward'] >=0 ? $goods['goodsReward'] : 0;
        }
