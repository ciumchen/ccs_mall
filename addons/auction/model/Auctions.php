...
class Auctions extends Base{
	...
	public function beforeCancelOrder($params){
		$order = DB::name('orders')->where('orderId',$params['orderId'])->field('orderCode,orderNo')->find();
		if($order['orderCode']=='auction'&&$order['orderNo']!='100012931')die('{"status":-1,"msg":"对不起，拍卖订单不允许取消"}');
	}
